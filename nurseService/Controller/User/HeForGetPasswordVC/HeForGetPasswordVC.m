...
- (IBAction)getCodeButtonClick:(UIButton *)sender
{
    ...
    [AFHttpTool requestWihtMethod:RequestMethodTypePost url:@"nurseAnduser/ResetPasswordSMSVerification.action" params:params success:^(AFHTTPRequestOperation* operation,id response){
        [self hideHud];
...
- (IBAction)resetButtonClick:(UIButton *)sender
{
    ...
    [AFHttpTool requestWihtMethod:RequestMethodTypePost url:@"nurseAnduser/ResetPasswordComparison.action" params:params success:^(AFHTTPRequestOperation* operation,id response){
        [self hideHud];
        NSString *respondString = [[NSString alloc] initWithData:operation.responseData encoding:NSUTF8StringEncoding];
        NSDictionary *respondDict = [NSDictionary dictionaryWithDictionary:[respondString objectFromJSONString]];
        NSInteger errorCode = [[respondDict objectForKey:@"errorCode"] integerValue];
