...
- (TTCheckBoxButton *)tagButtonWithTag:(NSString *)tag {
    ...
    btnFrame.size.width = [tagBtn.titleLabel.text sizeWithAttributes:@{NSFontAttributeName:_fontTag}].width + (tagBtn.layer.cornerRadius * 16.0f);
...
- (void)addTagToLast:(NSString *)tag {
    ...
    if (result.count == 0) {
        ...
        [_svContainer scrollRectToVisible:CGRectMake(_svContainer.frame.origin.x, _svContainer.contentSize.height - 50, SCREENWIDTH, 50) animated:YES];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    frame.size.width = MAX(frame.size.width, _tagWidht);
    if (frame.size.width + _tagPaddingSize.width * 2 >= _svContainer.frame.size.width - 30) {
        ...
        if (string2.length < textField.text.length) {
            ...
            frame.size.width = MIN(frame.size.width, _svContainer.frame.size.width);
            _tfInput.frame = frame;
            return NO;
        }
