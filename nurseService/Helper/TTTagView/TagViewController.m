...
@interface TagViewController ()<TTTagViewDelegate, TTGroupTagViewDelegate, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate>
...
- (UITableView *)tableView {
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.inputHeight, SCREENWIDTH , SCREENHEIGH - (64 + self.inputHeight)) style:UITableViewStyleGrouped];
    }
    return _tableView;
}
...
//    self.dataArr = [NSMutableArray arrayWithArray:@[@[@"资讯", @"22", @"哈哈", @"我是标签", @"我也可以换行"], @[@"333", @"4444"], @[@"55555", @"666666"], @[@"11", @"222"], @[@"3333", @"44444"], @[@"555555", @"6666666"], @[@"111", @"2222"], @[@"33333", @"444444"], @[@"5555555", @"66666666"]]];
    NSMutableArray *tag = @[].mutableCopy;
    self.titleArr = @[].mutableCopy;
    for (NSDictionary *tagDic in self.tagListArr) {
        NSString *twoTagArrString = [tagDic objectForKey:@"postTwoInfo"];
        NSData *data = [twoTagArrString dataUsingEncoding:NSUTF8StringEncoding];
        NSArray *twoTagArr = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableLeaves error:nil];
        NSMutableArray *twoArr = @[].mutableCopy;
        if ([twoTagArr count]>0) {
            for (NSDictionary *twoDic in twoTagArr) {
                [twoArr addObject:[twoDic objectForKey:@"PostTwoLevelName"]];
            }
            [tag addObject:twoArr];
            [self.titleArr addObject:[tagDic objectForKey:@"postOneLevelName"]];
        }
    }
    NSLog(@"所有的tag是%@",tag);
    self.dataArr = tag;
...
- (void)addSubviews {
    ...
    textBgView.layer.borderColor = [UIColor clearColor].CGColor;
    textBgView.layer.borderWidth = 0.5;
    textBgView.backgroundColor = [UIColor whiteColor];
    
    [self.view addSubview:textBgView];
    self.textBgView = textBgView;
    textBgView.frame = CGRectMake(0, 0, SCREENWIDTH, self.inputHeight);
    
    inputTagView = [[TTTagView alloc] initWithFrame:CGRectMake(0, 0,self.view.frame.size.width ,self.inputHeight)];
    inputTagView.translatesAutoresizingMaskIntoConstraints=YES;
    inputTagView.delegate = self;
    inputTagView.tfInput.hidden = YES;
    
    {// 这些属性有默认值, 可以不进行设置
//        inputTagView.inputBgColor = kRandomColor;
//        inputTagView.inputPlaceHolderTextColor = kRandomColor;
//        inputTagView.inputTextColor = APPDEFAULTTITLECOLOR;
//        inputTagView.inputBorderColor = kRandomColor;
//        inputTagView.bgColor = kRandomColor;
        inputTagView.textColor = APPDEFAULTTITLECOLOR;
        inputTagView.borderColor = APPDEFAULTTITLECOLOR;
...
//        inputTagView.selBorderColor = kRandomColor;
    ...
    self.tableView.backgroundColor = APPDEFAULTTABLEBACKGROUNDCOLOR;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    bgView.backgroundColor = APPTABLESEPARATORCOLOR;
...
//    [bgView addSubview:view];
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, 10, SCREENWIDTH - 35, 20)];
    if (self.dataArr.count > 0) {
        label.text = [self.titleArr objectAtIndex:section];
    }
...
- (TTGroupTagView *)addHistoryViewTagsWithCGRect:(CGRect)rect andIndex:(NSIndexPath *)indexPath{
    ...
    {// 这些属性颜色有默认值, 可以设置也可以不设置
        tagView.bgColor = [UIColor clearColor];
        tagView.textColor = [UIColor grayColor];
        tagView.borderColor = [UIColor grayColor];
        tagView.selBgColor = [UIColor clearColor];
        tagView.selTextColor = APPDEFAULTTITLECOLOR;
        tagView.selBorderColor = APPDEFAULTTITLECOLOR;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.textBgView.frame = CGRectMake(0, 0, SCREENWIDTH, self.inputHeight);
    }];
    [UIView animateWithDuration:0.25 animations:^{
        self.tableView.frame = CGRectMake(0, self.inputHeight, SCREENWIDTH, SCREENHEIGH - self.inputHeight - 64);
    }];
